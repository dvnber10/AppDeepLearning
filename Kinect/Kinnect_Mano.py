import sys, time, math, os
import cv2
import numpy as np
import freenect
from PIL import Image
from PyQt5.QtCore import Qt, QPoint, QRect, QTimer
from PyQt5.QtGui import QPixmap, QPainter, QPen, QColor, QFont, QImage
from PyQt5.QtWidgets import QApplication, QWidget, QLabel

# =========================
# NUEVAS IMPORTACIONES PARA COMUNICACIÓN Y TTS
# =========================
import requests
import json
import threading
import serial
import unicodedata
import platform

# --- Configuración Tesseract (Asegúrate de que la ruta sea correcta) ---
os.environ['TESSDATA_PREFIX'] = '/usr/share/tessdata/'  # Configurar TESSDATA_PREFIX para Arch Linux
import pytesseract
pytesseract.pytesseract.tesseract_cmd = '/usr/bin/tesseract'  # Ruta del ejecutable en Arch Linux
from extracciónTexto import extract_text_from_image_1 # Ya no se usa la librería TrOCR
from text_to_speech import text_to_speech_file_pyttsx3 # Ya no se usa pyttsx3

# --- CONFIGURACIÓN SERIAL Y ELEVENLABS ---
# ⚠️ ADVERTENCIA: Cambia 'COM4' por el puerto correcto de tu ESP32 o '/dev/ttyUSB0' en Linux
PORT_ESP32 = 'COM4'       
BAUDRATE = 115200          
ELEVENLABS_API_KEY = "sk_2f185333dfcb8bb4f87016fbb1769644426c0b3ee7fb9580"  
ELEVENLABS_VOICE_ID = "21m00Tcm4TlvDq8ikWAM"  # Voz Rachel
OUTPUT_AUDIO_FILE = "tts_kinect_output.mp3"
OUTPUT_TEXT_FILE = "texto_kinect_extraido.txt"

# --- CONFIGURACIÓN SERIAL ---
ser = None
try:
    ser = serial.Serial(PORT_ESP32, BAUDRATE, timeout=1)
    time.sleep(2)
    print(f"✅ Conexión con ESP32 en {PORT_ESP32} exitosa.")
except Exception as e:
    print(f"❌ ERROR al conectar con ESP32 en {PORT_ESP32}: {e}")
    print("La comunicación serial NO estará disponible.")

# =========================
# NUEVAS UTILIDADES (Adaptadas del script de la ESP32)
# =========================

def normalize_text(text):
    """Normaliza el texto eliminando tildes y caracteres especiales para la ESP32."""
    normalized = unicodedata.normalize('NFD', text)
    # Solo caracteres alfanuméricos y espacio
    clean_text = ''.join(c for c in normalized if unicodedata.category(c) != 'Mn' and (c.isalnum() or c.isspace()))
    return clean_text.upper().strip()

def send_to_esp32(text):
    """Envía el texto completo a la ESP32."""
    if ser and ser.is_open:
        try:
            # 1. Normalizar y quitar espacios (como en tu script original)
            clean_text = normalize_text(text).replace(" ", "")
            print(f"📤 Enviando a ESP32: {clean_text}")
            
            # 2. Enviar cada caracter con un retardo
            for char in clean_text:
                ser.write(char.encode('ascii'))
                time.sleep(0.3)
                # print(f"-> Enviado: {char}") # Descomenta para debug
            
            print(f"✅ Texto ({clean_text}) enviado completamente a ESP32.")
            return True
        except Exception as e:
            print(f"❌ Error al enviar datos a ESP32: {e}")
            return False
    else:
        print("⚠️ ESP32 no conectada. No se pudo enviar el texto.")
        return False

def generate_tts_audio(text):
    """Genera audio ultra-realista usando ElevenLabs API y lo guarda."""
    try:
        print(f"⏳ Generando audio ElevenLabs para: '{text}'...")
        if not ELEVENLABS_API_KEY or ELEVENLABS_API_KEY.startswith("TU_API_KEY"):
             print("❌ ERROR: API Key de ElevenLabs no configurada.")
             return False

        payload = {
            "text": text,
            "model_id": "eleven_multilingual_v2",
            "voice_settings": {"stability": 0.3, "similarity_boost": 0.7, "style": 0.2, "use_speaker_boost": True}
        }
        headers = {
            "xi-api-key": ELEVENLABS_API_KEY,
            "Content-Type": "application/json",
            "accept": "audio/mpeg"
        }
        
        response = requests.post(
            f"https://api.elevenlabs.io/v1/text-to-speech/{ELEVENLABS_VOICE_ID}",
            json=payload, headers=headers, timeout=30
        )
        
        if response.status_code == 200:
            with open(OUTPUT_AUDIO_FILE, "wb") as f:
                f.write(response.content)
            print(f"✅ Audio ElevenLabs guardado: {OUTPUT_AUDIO_FILE} ({os.path.getsize(OUTPUT_AUDIO_FILE)} bytes)")
            return True
        else:
            error_msg = f"❌ Error ElevenLabs API (Código {response.status_code}): "
            try: error_detail = response.json(); error_msg += error_detail.get('detail', str(error_detail))
            except: error_msg += response.text
            print(error_msg)
            return False
            
    except Exception as e:
        print(f"❌ Error inesperado en ElevenLabs: {e}")
        return False

def play_audio(file_path):
    """Reproduce el archivo MP3 usando el reproductor de audio nativo del sistema."""
    try:
        if not os.path.exists(file_path):
            print(f"❌ Archivo de audio no encontrado: {file_path}")
            return

        print("🔈 Reproduciendo audio ultra-realista...")
        sys_platform = platform.system()
        
        if sys_platform == "Windows": os.startfile(file_path)
        elif sys_platform == "Darwin": os.system(f"afplay '{file_path}'")
        elif sys_platform == "Linux": os.system(f"mpg123 '{file_path}'")
        else: print("⚠️ Plataforma no soportada para reproducción automática.")

        # No hay necesidad de esperar aquí, la reproducción suele ser asíncrona
    except Exception as e:
        print(f"❌ Error al reproducir audio: {e}")

# =========================
# Configuración principal
# =========================
CANVAS_W, CANVAS_H = 1200, 700
SIDEBAR_W = 200
FPS = 15
CURSOR_RADIUS = 10
PEN_WIDTH = 20  # Aumentado a 20 para texto más grueso
ERASER_WIDTH = 26
BTN_H = 60
BTN_PADDING = 10
BTN_W = SIDEBAR_W - 2 * BTN_PADDING
DWELL_MS = 1000
EMA_POS = 0.5
EMA_Z = 0.25
NEAR_MM = 300
FAR_MM = 1500
MIN_HAND_AREA = 150
MAX_HAND_AREA = 5000
FIST_CONVEXITY_THRESHOLD = 0.85
COLORS = [
    QColor(0, 0, 0),
    QColor(220, 50, 47),
    QColor(38, 139, 210),
    QColor(133, 153, 0),
    QColor(211, 54, 130),
    QColor(42, 161, 152)
]
COLOR_NAMES = ["Negro", "Rojo", "Azul", "Verde", "Rosa", "Turquesa"]

# =========================
# Utilidades Kinect (Mantenidas)
# =========================
def get_depth_mm():
    for _ in range(3):
        try:
            depth, _ = freenect.sync_get_depth(format=freenect.DEPTH_MM)
            rgb, _ = freenect.sync_get_video()
            rgb = cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR)
            if depth is not None and rgb is not None:
                return depth.astype(np.uint16), rgb
        except Exception:
            pass
        depth11, _ = freenect.sync_get_depth()
        rgb, _ = freenect.sync_get_video()
        rgb = cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR)
        if depth11 is not None and rgb is not None:
            depth_converted = (2047 - depth11).astype(np.uint16) * 2
            return depth_converted, rgb
    return None, None

def visualize_detection(rgb, depth_mm, hand_mask, hx=None, hy=None):
    """
    Crea una imagen combinada: RGB de fondo, profundidad superpuesta, y máscara de mano.
    Retorna una imagen RGB escalada para el canvas.
    """
    if depth_mm is None or rgb is None:
        return None
    
    h, w = depth_mm.shape
    
    # Visualizar profundidad: normaliza a colores (azul cerca, rojo lejos) para mejor visibilidad
    depth_vis = cv2.normalize(depth_mm, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
    depth_vis = cv2.applyColorMap(depth_vis, cv2.COLORMAP_JET)  # Colores: azul (cerca) a rojo (lejos)
    
    # Superponer profundidad sobre RGB (50% opacidad)
    depth_overlay = cv2.addWeighted(rgb, 0.7, depth_vis, 0.3, 0)
    
    # Superponer máscara de mano en rojo si existe
    if hand_mask is not None:
        # Expandir hand_mask a 3 canales para multiplicar
        hand_mask_3ch = cv2.cvtColor(hand_mask, cv2.COLOR_GRAY2BGR)
        hand_color = COLORS[1].getRgb()[:3]  # Rojo
        # Aplicar máscara al hand_color
        hand_masked = cv2.bitwise_and(hand_color, hand_color, mask=hand_mask)
        # Superponer
        depth_overlay = cv2.addWeighted(depth_overlay, 0.8, hand_masked, 0.2, 0)
    
    # Dibujar círculo en posición de mano detectada
    if hx is not None and hy is not None:
        cv2.circle(depth_overlay, (hx, hy), 10, (255, 255, 255), 2)  # Círculo blanco para centro de mano
    
    # Escalar a un tamaño fijo para el canvas (ej. 300x224, ajusta según necesites)
    vis_resized = cv2.resize(depth_overlay, (300, 224))
    return vis_resized

def find_hand(depth_mm):
    if depth_mm is None:
        mask = np.zeros((1,1), dtype=np.uint8)
        hand_mask = np.zeros_like(mask, dtype=np.uint8)
        return None, None, None, mask, hand_mask, False
    mask = np.zeros_like(depth_mm, dtype=np.uint8)
    mask[(depth_mm >= NEAR_MM) & (depth_mm <= FAR_MM)] = 255
    valid_pixels = depth_mm[(depth_mm >= NEAR_MM) & (depth_mm <= FAR_MM)]
    if valid_pixels.size == 0:
        hand_mask = np.zeros_like(mask, dtype=np.uint8)
        return None, None, None, mask, hand_mask, False
    min_val = np.min(valid_pixels)
    hand_mask = np.zeros_like(depth_mm, dtype=np.uint8)
    hand_mask[(depth_mm >= min_val) & (depth_mm <= min_val + 50)] = 255
    hand_mask = cv2.medianBlur(hand_mask, 5)
    kernel = np.ones((5,5), np.uint8)
    hand_mask = cv2.morphologyEx(hand_mask, cv2.MORPH_CLOSE, kernel)
    cnts, _ = cv2.findContours(hand_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    if not cnts:
        return None, None, None, mask, hand_mask, False
    valid_cnts = [cnt for cnt in cnts if MIN_HAND_AREA <= cv2.contourArea(cnt) <= MAX_HAND_AREA]
    if not valid_cnts:
        return None, None, None, mask, hand_mask, False
    best_cnt = max(valid_cnts, key=cv2.contourArea)
    M = cv2.moments(best_cnt)
    if M["m00"] == 0:
        return None, None, None, mask, hand_mask, False
    cx = int(M["m10"]/M["m00"])
    cy = int(M["m01"]/M["m00"])
    h, w = depth_mm.shape
    r = 8
    x0, x1 = max(0, cx - r), min(w, cx + r + 1)
    y0, y1 = max(0, cy - r), min(h, cy + r + 1)
    z_patch = depth_mm[y0:y1, x0:x1]
    if z_patch.size == 0:
        return None, None, None, mask, hand_mask, False
    z_val = int(np.median(z_patch[z_patch > 0])) if np.any(z_patch > 0) else None
    is_fist = False
    if len(best_cnt) >= 5:
        hull = cv2.convexHull(best_cnt)
        hull_area = cv2.contourArea(hull)
        cnt_area = cv2.contourArea(best_cnt)
        if hull_area > 0:
            convexity = cnt_area / hull_area
            is_fist = convexity < FIST_CONVEXITY_THRESHOLD
    return cx, cy, z_val, mask, hand_mask, is_fist

# =========================
# Interfaz y lógica (Modificada)
# =========================
class KinectPaint(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Kinect Paint - Mano Abierta (Mover) / Cerrada (Dibujar)")
        self.setFixedSize(CANVAS_W + SIDEBAR_W, CANVAS_H)
        self.canvas = QPixmap(CANVAS_W, CANVAS_H)
        self.canvas.fill(Qt.white)
        self.view = QLabel(self)
        self.view.setGeometry(0, 0, CANVAS_W + SIDEBAR_W, CANVAS_H)
        self.mode = "draw"
        self.color_idx = 0
        self.pen_down = False
        self.last_pt = None
        self.cur_x = CANVAS_W // 2 + SIDEBAR_W
        self.cur_y = CANVAS_H // 2
        self.cur_z = None
        print("Initialized cur_x:", self.cur_x, "cur_y:", self.cur_y, "cur_z:", self.cur_z)
        self.hover_start = None
        self.hover_target = None
        self.last_button_press = 0
        self.button_cooldown = 1.0
        self.buttons = self._make_buttons()
        self.timer = QTimer(self)
        self.timer.timeout.connect(self.update_frame)
        self.timer.start(int(1000 / FPS))
        self.font_big = QFont("Sans", 12, QFont.Bold)
        self.font_small = QFont("Sans", 10)
        self.current_vis = None
        self.is_processing = False # Nuevo flag para evitar múltiples procesos

    def _make_buttons(self):
        btns = []
        names = [
            ("Dibujar", "draw"),
            ("Borrar", "erase"),
            ("Guardar/Enviar", "save"), # Etiqueta modificada
            ("Limpiar", "clear"),
            ("Color Negro", "color0"),
            ("Color Rojo", "color1"),
            ("Color Azul", "color2"),
            ("Color Verde", "color3"),
            ("Color Rosa", "color4"),
            ("Color Turquesa", "color5"),
        ]
        x = BTN_PADDING
        y = BTN_PADDING
        for label, action in names:
            rect = QRect(x, y, BTN_W, BTN_H)
            btns.append((rect, label, action))
            y += BTN_H + BTN_PADDING
        return btns

    def kinect_to_canvas(self, x_depth, y_depth, shape):
        dh, dw = shape
        total_width = CANVAS_W + SIDEBAR_W
        sx = total_width / float(dw)
        sy = CANVAS_H / float(dh)
        return int((dw - x_depth) * sx), int(y_depth * sy)

    def update_frame(self):
        # ... (código update_frame sin cambios)
        depth_mm, rgb = get_depth_mm()
        hx, hy, hz, _mask, hand_mask, is_fist = find_hand(depth_mm)
        self.current_vis = None
        if hx is not None and hy is not None and hz is not None:
            cx, cy = self.kinect_to_canvas(hx, hy, depth_mm.shape)
            if not hasattr(self, 'cur_x'):
                self.cur_x = CANVAS_W // 2 + SIDEBAR_W
            if not hasattr(self, 'cur_y'):
                self.cur_y = CANVAS_H // 2
            self.cur_x = int(EMA_POS * cx + (1-EMA_POS) * self.cur_x)
            self.cur_y = int(EMA_POS * cy + (1-EMA_POS) * self.cur_y)
            if self.cur_z is None:
                self.cur_z = hz
            else:
                self.cur_z = int(EMA_Z * hz + (1-EMA_Z) * self.cur_z)
            self.apply_gestures(is_fist)
            try:
                self.current_vis = visualize_detection(rgb, depth_mm, hand_mask, hx, hy)
            except Exception as e:
                print(f"Error in visualize_detection: {e}")
                if rgb is not None:
                    self.current_vis = cv2.resize(rgb, (300, 224))
        else:
            self.pen_down = False
            self.last_pt = None
            self.hover_target = None
            self.hover_start = None
            if rgb is not None:
                self.current_vis = cv2.resize(rgb, (300, 224))
        self.render_scene(is_fist if hx is not None else None)

    def apply_gestures(self, is_fist):
        # ... (código apply_gestures sin cambios)
        target_btn = None
        for rect, _, action in self.buttons:
            if rect.contains(self.cur_x, self.cur_y):
                target_btn = action
                break
        now = time.time()
        if now - self.last_button_press < self.button_cooldown:
            self.hover_target = None
            self.hover_start = None
            return
        if target_btn != self.hover_target:
            self.hover_target = target_btn
            self.hover_start = now if target_btn else None
        else:
            if self.hover_target is not None and self.hover_start is not None:
                dwell_ms = (now - self.hover_start) * 1000
                if dwell_ms >= DWELL_MS:
                    self.activate_action(self.hover_target)
                    self.hover_start = None
                    self.hover_target = None
                    self.last_button_press = now
        if self.cur_x >= SIDEBAR_W and self.hover_target is None:
            self.pen_down = is_fist
        else:
            self.pen_down = False
            self.last_pt = None
        if self.pen_down and self.cur_x >= SIDEBAR_W:
            canvas_x = self.cur_x - SIDEBAR_W
            self.draw_at(QPoint(canvas_x, self.cur_y))
        else:
            self.last_pt = None

    def activate_action(self, action):
        if action == "draw":
            self.mode = "draw"
        elif action == "erase":
            self.mode = "erase"
        elif action == "save":
            # Si el proceso no está corriendo, iniciar
            if not self.is_processing:
                self.auto_save() # auto_save ahora inicia el hilo
        elif action.startswith("color"):
            self.color_idx = int(action[5:])
        elif action == "clear":
            self.clear_canvas()
        self.update_button_labels()

    def update_button_labels(self):
        self.buttons = self._make_buttons()

    def clear_canvas(self):
        self.canvas.fill(Qt.white)

    def draw_at(self, pt: QPoint):
        # ... (código draw_at sin cambios)
        painter = QPainter(self.canvas)
        if self.mode == "erase":
            pen = QPen(Qt.white, ERASER_WIDTH, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin)
        else:
            pen = QPen(COLORS[self.color_idx], PEN_WIDTH, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin)
        painter.setPen(pen)
        if self.last_pt is None:
            painter.drawLine(pt, pt)
        else:
            painter.drawLine(self.last_pt, pt)
        painter.end()
        self.last_pt = QPoint(pt)

    def auto_save(self):
        """Guarda la imagen e inicia el procesamiento en un hilo separado."""
        ts = time.strftime("%Y%m%d-%H%M%S")
        out = f"dibujo_{ts}.png"
        if self.canvas.save(out, "PNG"):
            print(f"Imagen guardada como: {out}")
            # --- CAMBIO CLAVE: Iniciar el procesamiento en un hilo ---
            self.is_processing = True
            threading.Thread(target=self._process_image_and_send, args=(out,)).start()
            return out
        else:
            print(f"Error al guardar la imagen: {out}")
            return None
        
    def extract_text_from_image(self, image_path):
        try:
            print(f"Intentando abrir imagen: {image_path}")
            img = Image.open(image_path)
            print("Imagen cargada correctamente")
            img = img.resize((img.width * 2, img.height * 2), Image.LANCZOS)
            img = img.convert('L')
            img_np = np.array(img)
            img_np = cv2.GaussianBlur(img_np, (5, 5), 0)
            img_np = cv2.adaptiveThreshold(img_np, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, 
                                          cv2.THRESH_BINARY_INV, 11, 2)
            kernel = np.ones((3, 3), np.uint8)
            img_np = cv2.dilate(img_np, kernel, iterations=1)
            img = Image.fromarray(img_np)
            text = extract_text_from_image_1(image_path)
            print(f"Texto extraído: {text}")
            txt_filename = os.path.splitext(image_path)[0] + ".txt"
            print(f"Guardando texto en: {txt_filename}")
            with open(txt_filename, 'w', encoding='utf-8') as f:
                f.write(text)
            print(f"Texto extraído y guardado en: {txt_filename}")
            text_to_speech_file_pyttsx3(txt_filename)
            return text
        except Exception as e:
            print(f"Error al extraer texto de la imagen: {e}")
            return None


    def _perform_ocr(self, image_path):
        """
        Realiza el OCR con pre-procesamiento de imagen (usando OpenCV y Tesseract).
        """
        try:
            print("🔬 Iniciando pre-procesamiento de imagen para OCR...")
            
            # 1. Cargar imagen y re-escalar (mejora la precisión para texto a mano)
            img = Image.open(image_path).convert('RGB')
            img = img.resize((img.width * 2, img.height * 2), Image.LANCZOS)
            
            # 2. Pre-procesamiento con OpenCV (adaptado de tu script original)
            img_np = np.array(img.convert('L')) # Escala de grises
            img_np = cv2.GaussianBlur(img_np, (5, 5), 0)
            img_np = cv2.adaptiveThreshold(img_np, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, 
                                          cv2.THRESH_BINARY_INV, 11, 2)
            kernel = np.ones((3, 3), np.uint8)
            img_np = cv2.dilate(img_np, kernel, iterations=1)
            
            # 3. OCR con Tesseract
            text = pytesseract.image_to_string(Image.fromarray(img_np), lang='spa')
            
            return text.strip()
        
        except Exception as e:
            print(f"❌ Error en el proceso de OCR (PyTesseract): {e}")
            return ""

    def _process_image_and_send(self, image_path):
        """
        Función que corre en un hilo separado.
        Ejecuta OCR, TTS y envío a ESP32.
        """
        print("\n--- INICIANDO PROCESO ASÍNCRONO DE EXTRACCIÓN Y ENVÍO ---\n")
        try:
            # 1. Extracción de Texto (OCR)
            text = self._perform_ocr(image_path)
            
            if not text or len(text.strip()) < 2:
                print("⚠️ Texto extraído vacío o muy corto. Proceso cancelado.")
                return

            print(f"✨ Texto extraído: {text}")

            # 2. Guardar el texto en archivo
            txt_filename = os.path.splitext(image_path)[0] + ".txt"
            with open(txt_filename, 'w', encoding='utf-8') as f:
                f.write(text)
            print(f"📝 Texto guardado en: {txt_filename}")

            # 3. Envío a ESP32
            send_to_esp32(text)

            # 4. Generación y Reproducción de TTS (ElevenLabs)
            if generate_tts_audio(text):
                time.sleep(1) # Pequeña espera para asegurar que el archivo se termine de guardar
                play_audio(OUTPUT_AUDIO_FILE)
                
            print("\n--- PROCESO ASÍNCRONO FINALIZADO ---\n")

        except Exception as e:
            print(f"❌ Error al procesar la imagen y enviar: {e}")
        finally:
            self.is_processing = False # Resetear el flag al finalizar

    # La función extract_text_from_image original es eliminada/sustituida por _process_image_and_send
    
    def render_scene(self, is_fist):
        # ... (código render_scene con una pequeña modificación en el HUD)
        frame = QPixmap(self.width(), self.height())
        frame.fill(QColor(245, 245, 245))
        painter = QPainter(frame)
        sidebar_rect = QRect(0, 0, SIDEBAR_W, CANVAS_H)
        painter.fillRect(sidebar_rect, QColor(240, 240, 240))
        painter.drawPixmap(SIDEBAR_W, 0, self.canvas)
        
        # Agregar visualización de Kinect/detección en la parte inferior del canvas
        if self.current_vis is not None:
            # Convertir numpy a QPixmap
            h, w, c = self.current_vis.shape
            qimg = QImage(self.current_vis.data, w, h, w * c, QImage.Format_RGB888)
            pixmap_vis = QPixmap.fromImage(qimg)
            # Posición: inferior del canvas
            vis_pos = QPoint(SIDEBAR_W + 10, CANVAS_H - h - 10)
            painter.drawPixmap(vis_pos, pixmap_vis)
        
        for rect, label, action in self.buttons:
            bg = QColor(230, 230, 230)
            if action == "draw" and self.mode == "draw":
                bg = QColor(210, 235, 210)
            elif action == "erase" and self.mode == "erase":
                bg = QColor(235, 210, 210)
            elif action.startswith("color") and int(action[5:]) == self.color_idx:
                bg = COLORS[int(action[5:])].lighter(150)
            
            # Nuevo: resalta el botón de guardado si se está procesando
            if action == "save" and self.is_processing:
                bg = QColor(255, 165, 0) # Naranja mientras procesa
                
            if self.hover_target == action:
                bg = bg.lighter(120)
                
            painter.fillRect(rect, bg)
            painter.setPen(QPen(Qt.black, 1))
            painter.drawRect(rect)
            painter.setPen(Qt.black)
            painter.setFont(self.font_big)
            painter.drawText(rect, Qt.AlignCenter, label)
            if self.hover_target == action and self.hover_start is not None:
                frac = min(1.0, (time.time() - self.hover_start) * 1000.0 / DWELL_MS)
                prog_rect = QRect(rect.left(), rect.top(), int(rect.width() * frac), 4)
                painter.fillRect(prog_rect, QColor(60, 60, 60))
        
        painter.setFont(self.font_small)
        hud = f"Modo: {'Dibujar' if self.mode=='draw' else 'Borrar'} | Color: {COLOR_NAMES[self.color_idx]} | "
        if self.cur_z is not None:
            hud += f"Distancia: {self.cur_z}mm | "
            if is_fist is not None:
                hud += f"Mano: {'Cerrada (Dibujando)' if is_fist else 'Abierta (Moviendo)'} | "
        
        if self.is_processing:
             hud += " Procesando... (Guardando/TTS/ESP32)"
             
        painter.setPen(Qt.black)
        painter.drawText(SIDEBAR_W + 12, 24, hud)
        painter.setPen(QPen(Qt.black, 2))
        painter.setBrush(Qt.NoBrush)
        painter.drawEllipse(QPoint(self.cur_x, self.cur_y), CURSOR_RADIUS, CURSOR_RADIUS)
        if is_fist and self.cur_x >= SIDEBAR_W:
            painter.setBrush(COLORS[self.color_idx] if self.mode == "draw" else Qt.white)
            painter.drawEllipse(QPoint(self.cur_x, self.cur_y), CURSOR_RADIUS - 4, CURSOR_RADIUS - 4)
        else:
            painter.drawLine(self.cur_x - 8, self.cur_y, self.cur_x + 8, self.cur_y)
            painter.drawLine(self.cur_x, self.cur_y - 8, self.cur_x, self.cur_y + 8)
        painter.end()
        self.view.setPixmap(frame)

if __name__ == "__main__":
    os.environ.setdefault("QT_ENABLE_HIGHDPI_SCALING", "1")
    try:
        app = QApplication(sys.argv)
        w = KinectPaint()
        w.show()
        sys.exit(app.exec_())
    finally:
        # Cerrar el puerto serial al salir
        if ser and ser.is_open:
            ser.close()
            print("Puerto serial cerrado al salir.")